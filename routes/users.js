...
router.get('/reset/:token', (req, res)=> {
    User.findOne({resetPasswordToken: req.params.token, resetPasswordExpires : {$gt : Date.now() } })//If the URL is /reset/xyz9876, then req.params.token would be equal to 'xyz9876'
        .then(user => {//token is used to know the value of the product choosed for example 
            if(!user) {
...
router.get('/users/all', isAuthenticatedUser ,(req, res)=> {
    User.find({})
        .then(users => {
            res.render('./users/alluser', {users : users});//the 2 {} are used to create an object in javascript //hala2 bel ejs files fina na3mel <%user%> wnhot lvalue taba3on bel view 
        })//bel all user ejs sta3malna for each kermel temro2 3ala kel user w kamen sta3malna <tr> li ma3neta metel ka2ana 3am n2ello lal ejs enno for each user repeat this wel td hiye l cell taba3 l array
        .catch(err => {
            req.flash('error_msg', 'ERROR: '+err);
...
router.post('/forgot', (req, res, next)=> {
    ...
    async.waterfall([//is a control flow library for Node.js that helps manage asynchronous operations in a series or waterfall fashion.
        (done) => {
            crypto.randomBytes(20, (err , buf) => {//crypto.randomBytes. The generated token is then passed to the next function in the waterfall.
